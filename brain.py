...
<style>
:root{--c:#00d9ff;--cs:#0099cc;--g:rgba(0,217,255,0.25);--g2:rgba(0,217,255,0.08);--bg:#000000;--bg2:#060d14;--bg3:#0a1520;--txt:#e0f7ff;--sub:rgba(0,217,255,0.6)}
:root.female{--c:#ff2d78;--cs:#d91a5e;--g:rgba(255,45,120,0.25);--g2:rgba(255,45,120,0.08);--bg:#0a0008;--bg2:#110010;--bg3:#180018}
*{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent}
html,body{width:100%;height:100%;overflow:hidden;background:var(--bg);color:var(--txt);font-family:'Courier New',monospace}
body::before{content:'';position:fixed;top:0;left:0;width:100%;height:100%;background-image:linear-gradient(var(--g2) 1px,transparent 1px),linear-gradient(90deg,var(--g2) 1px,transparent 1px);background-size:40px 40px;pointer-events:none;z-index:0;opacity:0.5}
body::after{content:'';position:fixed;top:0;left:0;width:100%;height:100%;background:radial-gradient(ellipse at top,rgba(0,217,255,0.06) 0%,transparent 60%),radial-gradient(ellipse at bottom,rgba(0,100,180,0.04) 0%,transparent 60%);pointer-events:none;z-index:0}
:root.female body::after{background:radial-gradient(ellipse at top,rgba(255,45,120,0.07) 0%,transparent 60%),radial-gradient(ellipse at bottom,rgba(0,217,255,0.04) 0%,transparent 60%)}
.wrap{position:relative;z-index:1;width:100%;height:100%;display:flex;flex-direction:column;max-width:600px;margin:0 auto}
.hdr{position:relative;padding:14px 16px 10px;background:linear-gradient(180deg,rgba(0,0,0,0.95) 0%,rgba(6,13,20,0.9) 100%);border-bottom:1px solid var(--c);box-shadow:0 0 30px var(--g),0 2px 0 var(--c);flex-shrink:0}
.hdr::before,.hdr::after{content:'';position:absolute;width:18px;height:18px}
.hdr::before{top:6px;left:6px;border-top:2px solid var(--c);border-left:2px solid var(--c)}
.hdr::after{top:6px;right:6px;border-top:2px solid var(--c);border-right:2px solid var(--c)}
.hdr-row1{display:flex;align-items:center;justify-content:space-between;margin-bottom:6px}
.hdr-icon{width:36px;height:36px;border:1px solid var(--c);border-radius:10px;background:var(--g2);color:var(--c);font-size:16px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.2s}
.hdr-icon:hover{background:var(--g);box-shadow:0 0 12px var(--g)}
.hdr-title{display:flex;align-items:center;gap:12px}
.hdr-title h1{font-size:30px;font-weight:900;color:var(--c);text-shadow:0 0 20px var(--c),0 0 40px var(--g);letter-spacing:4px}
.hex-wrap{display:flex;gap:8px}
.hex{width:38px;height:36px;clip-path:polygon(50% 0%,100% 25%,100% 75%,50% 100%,0% 75%,0% 25%);display:flex;align-items:center;justify-content:center;font-size:15px;cursor:pointer;transition:all 0.2s;border:none;outline:none}
.hex.male{background:rgba(0,217,255,0.15);color:#00d9ff}
.hex.female-btn{background:rgba(255,45,120,0.15);color:#ff2d78}
.hex.active{background:var(--c);color:#000;box-shadow:0 0 20px var(--g)}
.hex.female-btn.active{background:#ff2d78;color:#000;box-shadow:0 0 20px rgba(255,45,120,0.5)}
.hdr-sub{text-align:center;font-size:10px;color:var(--c);letter-spacing:3px;opacity:0.8;text-transform:uppercase}
.hdr-sub span{opacity:0.5;margin:0 6px}
...
.chat::-webkit-scrollbar-thumb{background:var(--c);border-radius:10px}
.msg-wrap{display:flex;flex-direction:column;gap:3px;animation:fadeup 0.3s ease}
@keyframes fadeup{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
.msg-wrap.user{align-items:flex-end}
.msg-label{font-size:9px;letter-spacing:2px;opacity:0.7;color:var(--c);margin-bottom:2px;text-transform:uppercase}
.msg-wrap.user .msg-label{text-align:right}
.msg-bubble{position:relative;padding:12px 16px;max-width:85%;line-height:1.6;font-size:13px;word-break:break-word;white-space:pre-wrap}
.msg-wrap.user .msg-bubble{background:var(--bg3);border:1px solid var(--c);border-radius:18px 4px 18px 18px;box-shadow:0 0 15px var(--g),inset 0 0 20px var(--g2);color:var(--txt)}
.msg-wrap.aria .msg-bubble{background:var(--bg2);border:1px solid var(--c);border-radius:4px 18px 18px 18px;box-shadow:0 0 20px var(--g),inset 0 0 30px var(--g2);color:var(--txt)}
.msg-wrap.aria .msg-bubble p{margin:5px 0;line-height:1.6}
.msg-wrap.aria .msg-bubble ul{margin:6px 0 6px 18px}
.msg-wrap.aria .msg-bubble li{margin:2px 0}
.msg-wrap.aria .msg-bubble h1,.msg-wrap.aria .msg-bubble h2{font-size:11px;color:var(--c);text-transform:uppercase;letter-spacing:1px;margin:6px 0 4px;text-shadow:0 0 8px var(--g)}
.msg-wrap.aria .msg-bubble strong{color:var(--c)}
.msg-time{font-size:9px;color:var(--sub);letter-spacing:1px;display:flex;align-items:center;gap:5px;margin-top:2px}
.msg-wrap.user .msg-time{justify-content:flex-end}
.checkmark{color:var(--c);font-size:10px}
.aria-dot{width:7px;height:7px;border-radius:50%;background:var(--c);box-shadow:0 0 8px var(--c);display:inline-block;margin-right:6px;animation:pulse 2s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}
.typing-dots{display:flex;gap:5px;padding:4px 0}
.typing-dots span{width:6px;height:6px;border-radius:50%;background:var(--c);opacity:0.4;animation:dot-bounce 1.2s infinite}
.typing-dots span:nth-child(2){animation-delay:0.2s}
.typing-dots span:nth-child(3){animation-delay:0.4s}
@keyframes dot-bounce{0%,80%,100%{opacity:0.4;transform:scale(1)}40%{opacity:1;transform:scale(1.3)}}
.rate-bar{display:none;flex-direction:row;gap:6px;margin-top:5px;flex-wrap:wrap}
.rate-bar.show{display:flex}
.rate-btn{padding:5px 11px;background:var(--g2);border:1px solid var(--c);color:var(--c);font-size:12px;cursor:pointer;border-radius:20px;transition:all 0.2s}
.rate-btn:hover,.rate-btn.picked{background:var(--c);color:#000;box-shadow:0 0 10px var(--g)}
.rate-done{font-size:10px;color:var(--c);opacity:0.7;letter-spacing:1px;margin-top:2px}
.inp-wrap{position:relative;padding:12px 14px;background:rgba(0,0,0,0.95);border-top:1px solid var(--c);box-shadow:0 -2px 20px var(--g);flex-shrink:0}
.inp-wrap::before,.inp-wrap::after{content:'';position:absolute;width:14px;height:14px}
.inp-wrap::before{top:-1px;left:0;border-top:2px solid var(--c);border-left:2px solid var(--c)}
.inp-wrap::after{top:-1px;right:0;border-top:2px solid var(--c);border-right:2px solid var(--c)}
.inp-row{display:flex;gap:8px;align-items:flex-end;border:1px solid var(--c);border-radius:20px;background:var(--bg3);padding:6px 8px 6px 16px;box-shadow:0 0 15px var(--g),inset 0 0 20px var(--g2)}
textarea{flex:1;background:transparent;border:none;outline:none;color:var(--txt);font-family:'Courier New',monospace;font-size:13px;resize:none;min-height:36px;max-height:90px;line-height:1.5;padding:4px 0}
textarea::placeholder{color:var(--sub)}
.send-btn{padding:8px 20px;background:transparent;border:1px solid var(--c);border-radius:16px;color:var(--c);font-family:'Courier New',monospace;font-size:12px;font-weight:700;letter-spacing:2px;cursor:pointer;transition:all 0.2s;white-space:nowrap;text-transform:uppercase;box-shadow:0 0 10px var(--g)}
.send-btn:hover{background:var(--c);color:#000;box-shadow:0 0 20px var(--g)}
.send-btn:active{transform:scale(0.97)}
.ftr{text-align:center;padding:7px;font-size:9px;color:var(--c);letter-spacing:3px;opacity:0.5;border-top:1px solid rgba(0,217,255,0.2);flex-shrink:0}
</style>
</head>
<body>
<div class="wrap">
<div class="hdr">
  <div class="hdr-row1">
    ...
    <div class="hdr-title">
      <h1>ARIA</h1>
      <div class="hex-wrap">
        <button class="hex male active" id="btnM" onclick="setTheme('male')">&#9794;</button>
        <button class="hex female-btn" id="btnF" onclick="setTheme('female')">&#9792;</button>
      </div>
    </div>
    <div class="hdr-icon">&#9881;</div>
  </div>
  <div class="hdr-sub"><span>&raquo;</span> Strategic Thinking Partner <span>&laquo;</span></div>
</div>
<div class="chat" id="chat"></div>
<div class="inp-wrap">
  <div class="inp-row">
    <textarea id="inp" placeholder="Command..." rows="1"></textarea>
    <button class="send-btn" onclick="send()">SEND</button>
  </div>
</div>
<div class="ftr">&bull; LEARNING. ADAPTING. EVOLVING. &bull;</div>
</div>
<script>
const chat=document.getElementById("chat"),inp=document.getElementById("inp");
let UID=localStorage.getItem("aria_uid");
if(!UID){const name=prompt("What's your name?")||"user_"+Math.random().toString(36).substr(2,6);UID=name.toLowerCase().replace(/\\s+/g,"_")+Math.random().toString(36).substr(2,5);localStorage.setItem("aria_uid",UID);localStorage.setItem("aria_name",name)}
let lastMsgId=null;
function setTheme(t){const root=document.documentElement;document.getElementById("btnM").classList.toggle("active",t==="male");document.getElementById("btnF").classList.toggle("active",t==="female");t==="female"?root.classList.add("female"):root.classList.remove("female");localStorage.setItem("aria_theme",t)}
function now(){return new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}
function addMsg(text,sender,showRating=false){
  ...
  label.innerHTML=sender==="user"?"YOU":'<span class="aria-dot"></span>ARIA';
  const bubble=document.createElement("div");bubble.className="msg-bubble";
  if(sender==="aria"){bubble.innerHTML=text==="..."?'<div class="typing-dots"><span></span><span></span><span></span></div>':marked.parse(text)}
  else{bubble.textContent=text}
  const timeRow=document.createElement("div");timeRow.className="msg-time";
  timeRow.innerHTML=sender==="user"?now()+' <span class="checkmark">&#10003;&#10003;</span>':now();
  wrap.appendChild(label);wrap.appendChild(bubble);wrap.appendChild(timeRow);
